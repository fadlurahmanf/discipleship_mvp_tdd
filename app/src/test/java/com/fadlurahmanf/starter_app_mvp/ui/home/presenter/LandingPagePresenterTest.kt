...
class LandingPagePresenterTest{
    ...
    @Mock
    lateinit var userWithAuthEntity: UserWithAuthEntity
    ...
    fun beforeEach(){
        ...
        presenter = LandingPagePresenter(userWithAuthEntity, authRepository)
        presenter.view = view
    ...
    fun `get my group success get my subscription success`(){
        ...
        Mockito.`when`(userWithAuthEntity.getMyGroups()).thenReturn(
            Observable.just(groupResponse)
        )

        Mockito.`when`(userWithAuthEntity.getSubscription()).thenReturn(
            Observable.just(subscriptionResponse)
        )

        presenter.getMyGroupAndMySubscription()

        val inOrder = inOrder(view, userWithAuthEntity, authRepository)

        inOrder.verify(view, Mockito.times(1)).myGroupAndSubscriptionLoading()
        inOrder.verify(userWithAuthEntity, Mockito.times(1)).getMyGroups()
        inOrder.verify(userWithAuthEntity, Mockito.times(1)).getSubscription()
    ...
    fun `get my training success`(){
        ...
        val inOrder = inOrder(view, userWithAuthEntity)

        inOrder.verify(view, Mockito.times(1)).getTrainingLoading()
        inOrder.verify(userWithAuthEntity, Mockito.times(1)).getMyTrainingStudyTopic()
